...
public abstract class BlockModFluid extends Block
{
    protected BlockModFluid(int par1, Material par2Material)
    {
        super(par1, (par2Material == mod_CC.DarkWater ? 14 : 12) * 16 + 13, par2Material);
        float var3 = 0.0F;
        float var4 = 0.0F;
    ...
    {
        return this.blockMaterial != mod_CC.DarkWater;
    }
    ...
    {
        if (this.blockMaterial != mod_CC.HolyWater)
        {
            return 0x00FFFF;
        }
        else
    ...
    public int tickRate()
    {
        return this.blockMaterial == mod_CC.HolyWater ? 5 : (this.blockMaterial == mod_CC.DarkWater ? 30 : 0);
    }
    ...
    {
        return this.blockMaterial == mod_CC.HolyWater ? 1 : 0;
    }
    ...
    public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        if (this.blockMaterial == mod_CC.HolyWater && par5Random.nextInt(64) == 0)
        {
            var6 = par1World.getBlockMetadata(par2, par3, par4);
        ...
        if (this.blockMaterial == mod_CC.DarkWater && par1World.getBlockMaterial(par2, par3 + 1, par4) == Material.air && !par1World.isBlockOpaqueCube(par2, par3 + 1, par4))
        {
    ...
    public static double func_293_a(IBlockAccess par0IBlockAccess, int par1, int par2, int par3, Material par4Material)
    {
        ...
        if (par4Material == mod_CC.HolyWater)
        {
            var5 = ((BlockModFluid)mod_CC.HolyLiquidMoving).getFlowVector(par0IBlockAccess, par1, par2, par3);
        }

        if (par4Material == mod_CC.DarkWater)
        {
            var5 = ((BlockModFluid)mod_CC.DarkLiquidMoving).getFlowVector(par0IBlockAccess, par1, par2, par3);
        }
    ...
    private void checkForHarden(World par1World, int par2, int par3, int par4)
    {
        ...
        {
            if (this.blockMaterial == mod_CC.DarkWater)
            {
                boolean var5 = false;

                if (var5 || par1World.getBlockMaterial(par2, par3, par4 - 1) == mod_CC.HolyWater)
                {
                    var5 = true;
                }
                ...
                if (var5 || par1World.getBlockMaterial(par2 - 1, par3, par4) == mod_CC.HolyWater)
                {
                    var5 = true;
                }
                ...
                if (var5 || par1World.getBlockMaterial(par2, par3 + 1, par4) == mod_CC.HolyWater)
                {
                    var5 = true;
                }
                ...
                if (var5)
                {
                    ...
                    if (var6 == 0)
                    {
                        par1World.setBlockWithNotify(par2, par3, par4, Block.oreDiamond.blockID);
                    }
                    else if (var6 <= 4)
                    {
                        par1World.setBlockWithNotify(par2, par3, par4, Block.cobblestoneMossy.blockID);
                    }
